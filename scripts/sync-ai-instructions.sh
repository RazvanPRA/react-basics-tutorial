#!/bin/sh
# Sync GPT.md (source of truth) into .github/copilot-instructions.md.
set -eu
...
Usage: scripts/sync-ai-instructions.sh [--check]
  ...
  (no args)  Regenerate .github/copilot-instructions.md from GPT.md.
...
if [ "$MODE" = check ]; then
  ...
  if ! diff -u "$TARGET" "$EXPECTED"; then
    echo "stale: .github/copilot-instructions.md does not match GPT.md" >&2
    echo "run ./scripts/sync-ai-instructions.sh" >&2
    exit 1
  fi
  echo "up to date"
...
mkdir -p "$ROOT/.github"
generate >"$TARGET"
echo "synced: GPT.md -> .github/copilot-instructions.md"